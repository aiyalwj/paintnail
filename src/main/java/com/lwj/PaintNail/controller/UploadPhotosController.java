...
//    图片是以content-type为multipart/form-data的格式上传的，所以使用spring-mvc可以通过使用参数的形式以二进制的格式获取到该图片。
    public String upload(HttpServletRequest request, @RequestParam(value = "file", required = false) MultipartFile file) throws IOException {
        ...
        if(!file.isEmpty()) {
            ...
            if (type != null) {
                if ("GIF".equals(type.toUpperCase())||"PNG".equals(type.toUpperCase())||"JPG".equals(type.toUpperCase())||"MP4".equals(type.toUpperCase())) {
...
//                    String trueFileName =pdNo+"-"+date+fileName.substring(fileName.lastIndexOf("."));
                    if("MP4".equals(type.toUpperCase()))    {  trueFileName =pdNo+"-"+date+".mp4"; }
                    else    { trueFileName =pdNo+"-"+date+".jpg";} //把图片都变成jpg格式，按需求决定该不该格式
                    log.info("文件自定义名称为：" + trueFileName + " 类型为：" + type);
                    // 设置存放文件的路径
                    path = realPath +trueFileName;
                    log.info("存放图片或视频文件的路径:" + path);
                    //判断文件父目录是否存在
